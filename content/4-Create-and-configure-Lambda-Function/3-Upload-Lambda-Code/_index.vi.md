...
**Mục tiêu:** Cung cấp code Python có thể thực thi cho hàm Lambda, định nghĩa logic để phát hiện lỗi máy chủ chính và điều phối việc gán lại Elastic IP cho máy chủ dự phòng.
#### Các bước thực hiện
1. **Truy cập trình soạn thảo code của hàm Lambda:** Trong cấu hình của `SDNEIPFailoverFunction`, chọn tab **Code**.
2. **Thay thế code mặc định:** Xóa bất kỳ code mặc định nào có sẵn trong cửa sổ soạn thảo `lambda_function.py`.

    ![image.png](image.png)

3. **Dán code được cung cấp:** Chèn code Python sau vào trình soạn thảo `lambda_function.py`:
    ...
    def lambda_handler(event, context):
        ...
        if not instance_id_in_alarm and 'detail' in event and 'state' in event['detail'] and 'reasonData' in event['detail']['state']:
            try:
                ...
                reason_data_json = json.loads(reason_data_str) # Parse the JSON string
...
4. **Deploy code:** Nhấn nút **Deploy** để lưu và triển khai các thay đổi cho hàm Lambda của bạn.
